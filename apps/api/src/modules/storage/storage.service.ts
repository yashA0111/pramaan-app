...
export class StorageService implements StoragePort {
  ...
  async uploadFile(
    ...
    try {
      ...
      this.logger.warn(
        `Primary storage upload failed (${err.message}); retrying with Local File Storage adapter.`,
      );
  ...
  async getFile(storagePath: string): Promise<{ buffer: Buffer; mimeType: string }> {
    try {
      ...
      if (this.adapter !== this.localAdapter) {
        try {
          return await this.localAdapter.getFile(storagePath);
        } catch {
          // Both failed
        }
      }
