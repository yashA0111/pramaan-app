    ...
    <main className="min-h-screen bg-[linear-gradient(135deg,hsl(var(--background)),hsl(var(--surface-muted)))] px-4 py-6 text-foreground md:py-10 print:bg-white print:p-0">
      <div className="mx-auto max-w-4xl print:max-w-none">
        <div className="mb-6 flex items-center justify-between gap-3 print:hidden">
          <Link to="/admin/demo" className="inline-flex min-h-10 items-center gap-2 border border-border bg-surface px-3 text-body-sm font-semibold text-foreground hover:bg-muted"><ArrowLeft className="size-4" aria-hidden="true" /> Admin</Link>
          <button type="button" onClick={() => window.print()} className="inline-flex min-h-10 items-center gap-2 bg-primary px-4 text-body-sm font-semibold text-primary-foreground hover:bg-primary/90"><Printer className="size-4" aria-hidden="true" /> Print ID card</button>
        </div>

        <article className="mx-auto aspect-[1.586/1] w-full max-w-[760px] overflow-hidden border border-primary/25 bg-surface shadow-elev-2 print:shadow-none">
          <div className="flex h-full flex-col p-5 sm:p-8">
            <header className="flex items-start justify-between border-b border-border pb-4 sm:pb-5">
              <div className="flex items-center gap-2.5"><PramaanMark className="size-8" /><div><p className="font-display text-card-title font-semibold tracking-tight">Pramaan</p><p className="text-[10px] uppercase tracking-[0.2em] text-foreground-subtle">Evidence becomes trust</p></div></div>
              <div className="text-right"><p className="font-display text-body-sm font-semibold uppercase tracking-[0.16em] text-primary">Official</p><p className="mt-1 text-[10px] uppercase tracking-widest text-foreground-subtle">Synthetic demonstration</p></div>
            </header>
            ...
            <div className="grid min-h-0 flex-1 grid-cols-[clamp(5rem,22%,9rem)_minmax(0,1fr)_clamp(5.5rem,24%,10.5rem)] items-center gap-4 py-5 sm:gap-7 sm:py-7">
              ...
              <div className="min-w-0 self-center">
                ...
                <div className="mt-5 border-t border-border pt-3"><p className="text-[10px] uppercase tracking-[0.2em] text-foreground-subtle">Credential reference</p><p className="mt-1 break-all font-display text-body-sm font-semibold tracking-wider text-foreground sm:text-credential">{credential.reference}</p></div>
              </div>
              <div className="justify-self-end text-center"><div className="bg-white p-2 sm:p-3">{qrDataUrl ? <img src={qrDataUrl} alt={`Permanent credential QR encoding ${permanentUri}`} className="size-[clamp(4.5rem,18vw,9rem)]" /> : <div className="size-[clamp(4.5rem,18vw,9rem)] bg-muted" />}</div><p className="mt-2 text-[9px] font-semibold uppercase tracking-widest text-foreground-subtle">Scan with Pramaan</p></div>
            </div>

            <footer className="flex items-center justify-between border-t border-border pt-3 text-[10px] text-foreground-subtle sm:pt-4"><span className="inline-flex items-center gap-1.5"><ShieldCheck className="size-3.5 text-success" aria-hidden="true" /> Credential status: {credential.status}</span><span>Valid until {new Date(credential.expiresAt).toLocaleDateString()}</span></footer>
          </div>
        </article>
        <p className="mx-auto mt-4 max-w-[760px] text-center text-metadata text-foreground-subtle print:hidden">This card is a synthetic demonstration artifact. The QR identifies the credential; it does not itself verify the person.</p>
      </div>
    </main>
  );
}
